...
@section('header-content')
    <section class="bg-DFEDFF  flex items-center justify-center h-20 px-4">

        <form action="{{route('panel.admin.user.search')}}"
              class=" px-2 min-w-full flex items-center justify-center  relative" method="POST">
            @csrf
            <input name="search" class="w-full py-2.5 px-4 shadow-lg rounded-md" type="text" placeholder="جستجو"/>
...
@section('content')
    <section class=" w-full bg-8EBFFC mt-4 space-y-10">
        @foreach($users->items() as $user)
            <article
                class=" space-y-3 flex-wrap rounded-md text-black  bg-white  p-2 flex items-center justify-between ">
                <div class="flex bg-red-100 items-center space-x-3 space-x-reverse min-w-full rounded-md p-4 ">
                    <div class="flex items-center justify-center flex-col space-y-2">
                        <img src="{{asset('merikhArz/src/images/usersAdmin.Avif')}}" alt="" class="w-16 h-16">
